...
​	wsssr 1.6.90驱动版本以上已优化性能（包含将读写自旋锁修改为rcu锁）

### 优化方式

​	代码修改：将读写自旋锁修改为rcu锁

​	wsssr研发人员反馈在网卡队列数高（比如24个）的情况下，流量比较大、数据包多的情况下比较容易发生这个软中断过高的问题，导致性能劣化。一般环境下不会有性能问题（网卡队列数为8的情况，流量比较小，数据包少的情况）。

### 优化效果

​	优化后的版本达到，低并发场景下，sysbench压测5%的性能劣化，其他并发场景无劣化。
...
​	wsssr为安全检查和流量监控软件，移动4a人员反馈安全要求必须安装，协调wsssr开发人员针对arm环境进行适配优化，优化后的版本达到低并发下，sysbench压测5%的性能劣化，其他并发场景无劣化的效果。
