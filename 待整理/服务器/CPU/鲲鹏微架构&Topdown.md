...
- Core Bound
  ...
  - Other Bound 其他整形指令  解决方法：循环展开
    ...
    - 0-ports-non-serialize 0条指令发射 （因为指令之间的依赖，可通过添加编译选项-funroll-loops）
    - X-ports X条指令发射
...
```shell
[root@localhost jesse]# ./kperf --hotfunc --topdown --cache --tlb --imix --uncore --duration 1 --interval 15 --pid 51465 
...
./kperf --hotfunc --topdown --cache --tlb --imix --uncore --verbose --duration 1 --interval 15 --pid 51465 --excel kperf
```
